...
// Interfaz para registros de Airtable
...
async function verifyActividades() {
  try {
    ...
    if (records.length > 0) {
      records.forEach((record: AirtableRecord, index: number) => {
        console.log(`\n--- Registro #${index + 1} ---`);
      ...
      records.forEach((record: AirtableRecord) => {
        if (record.fields['Empleados'] && Array.isArray(record.fields['Empleados'])) {
          record.fields['Empleados'].forEach((empId: string) => empleadosSet.add(empId));
        }
      });
