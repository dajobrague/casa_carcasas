...
function ScheduleEditor() {
  ...
  useEffect(() => {
    if (params?.storeId) {
      console.log('Usando Store ID de los parámetros de la ruta:', params.storeId);
      setStoreRecordId(params.storeId);
    } else {
      console.error('No se pudo encontrar el ID de la tienda en los parámetros de la ruta');
    }
    ...
    if (!storeRecordId) {
      return (
        <div className="bg-red-50 text-red-600 p-6 rounded-lg shadow-md">
          ...
          <p className="mb-4">
            Esta aplicación necesita el ID de la tienda en la URL.
            Por favor, asegúrese de que la URL tenga el formato correcto.
          </p>
          <p className="text-sm text-gray-700">
            Ejemplo: <code>https://tu-app.vercel.app/editor/recXXXXXXXXXXXXXX</code>
          </p>
        </div>
      );
    }
